...
from tropnn.layers.tropical import _pack_route_indices


def test_trop_linear_exact_route_index_packing_dtypes() -> None:
    indices = torch.tensor([[0, 7]], dtype=torch.long)

    assert _pack_route_indices(indices, cells=8).dtype == torch.uint8
    assert _pack_route_indices(indices, cells=1000).dtype == torch.int16
    assert _pack_route_indices(indices, cells=40000).dtype == torch.int64
