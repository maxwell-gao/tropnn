...
def _run_forward(
    ...
    *,
    ...
    if lut_dtype in {"int8", "fp8"}:
        ...
        forward_kernel = _pairwise_forward_4bit_block_kernel(item_count, out_features, tables, table_size, payload.data.shape[-1], block_d, out_blocks, target)
        forward_kernel(indices, payload.data, payload.scales, payload.codebook, output)
    else:
        payload_dtype_name = "bfloat16" if lut_dtype == "bf16" else "float32"
...
class _PairwiseTileLangFunction(torch.autograd.Function):
    ...
    def backward(ctx: Any, grad_output: Tensor, grad_indices: Tensor | None, grad_margins: Tensor | None) -> tuple[Any, ...]:
        ...
        if ctx.lut_dtype in {"fp32", "bf16"}:
            payload_dtype_name = "bfloat16" if ctx.lut_dtype == "bf16" else "float32"
            ...
            ste_kernel(grad_flat, indices_flat, margins_flat, anchors_contig, payload_data, grad_latent, grad_thresholds)
            ...
            ste_kernel(grad_flat, indices_flat, margins_flat, anchors_contig, payload_data, payload_scales, grad_latent, grad_thresholds)
        else:
            if ctx.use_min_margin_ste:
