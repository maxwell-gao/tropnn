...
if _HAS_TRITON:
    ...
    def _grad_margin_kernel(
        ...
        winner = tl.load(winner_ptr + offs_m * heads + head, mask=row_mask, other=0)
        runner = tl.load(runner_ptr + offs_m * heads + head, mask=row_mask, other=0)
    ...
    def _grad_code_kernel(
        ...
        for m0 in range(0, item_count, BLOCK_M):
            ...
            row_mask = offs_m < item_count
            winner = tl.load(winner_ptr + offs_m * heads + head, mask=row_mask, other=-1)
            runner = tl.load(runner_ptr + offs_m * heads + head, mask=row_mask, other=-1)
            margin = tl.load(margins_ptr + offs_m * heads + head, mask=row_mask, other=0.0)
...
def trop_exact_route_backward_triton(
    ...
    *,
    ...
    if winner_idx.shape != (item_count, heads) or runner_idx.shape != (item_count, heads) or margins.shape != (item_count, heads):
        raise ValueError("winner_idx, runner_idx, and margins must have shape [items, heads]")
    ...
    margin = margins.contiguous().to(torch.float32)
    ...
    grad_router_weight = torch.empty_like(weight)
    grad_router_bias = torch.empty((heads, cells), device=grad.device, dtype=torch.float32)
    grad_code = torch.empty_like(code_table)
